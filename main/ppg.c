...
// Section 1: b0=0.0205, b1=0.0410, b2=0.0205, a1=-1.5928, a2=0.6712
// Section 2: b0=1.0000, b1=2.0000, b2=1.0000, a1=-1.4409, a2=0.5421
// ============================================================
// IR channel (for beat detection)
...
#define BD_MASK_SAMPLES 20     // 200ms at 100Hz
#define BD_TIMEOUT_SAMPLES 200 // 2000ms at 100Hz
...
static float    bd_ema_interval; // in samples
...
void PPG_Signal_Process(uint16_t ir_raw, uint16_t red_raw)
{
    ...
    // Invert for beat detection (pulse falling edge → positive)
    float p = -y2;
    ...
    switch (bd_state) {
        ...
        case BD_WAITING:
            if (p > bd_threshold) {
                ...
                // No valid period — exponential decay
                if (bd_ema_interval < 1.0f) {
                    bd_threshold *= BD_GAMMA;
                } else {
                    // Linear decay toward falloff
        ...
        case BD_MAYBE:
            if (p + BD_DELTA < bd_threshold) {
                ...
                // Heart rate
                if (bd_ema_interval > 1.0f) {
                    // 100 Hz sample rate → 60000 / (interval * 10) = 6000 / interval
                    ppg_hr = (uint8_t)(6000.0f / bd_ema_interval + 0.5f);
                }

                // Reset threshold to fraction of peak
    ...
    // Timeout — if no beat for 2 seconds (200 samples), reset SpO₂ accumulators
